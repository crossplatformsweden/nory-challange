...
CREATE TABLE staff (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    -- No UNIQUE constraint on (location_id, name) assuming staff names might clash,
    -- relying on 'id' for uniqueness. Add if (location_id, name) must be unique.
);
...
COMMENT COMMENT ON COLUMN staff.location_id IS 'The ID of the location where the staff member primarily works.';
...
CREATE INDEX idx_staff_location_id ON staff (location_id); -- Index FK
...
CREATE INDEX idx_recipe_ingredient_links_recipe_id ON recipe_ingredient_links (recipe_id); -- Index FK
CREATE INDEX idx_recipe_ingredient_links_ingredient_id ON recipe_ingredient_links (ingredient_id); -- Index FK
...
CREATE INDEX idx_modifier_options_modifier_id ON modifier_options (modifier_id); -- Index FK
...
CREATE INDEX idx_location_menu_items_location_id ON location_menu_items (location_id); -- Index FK
CREATE INDEX idx_location_menu_items_recipe_id ON location_menu_items (recipe_id); -- Index FK
-- Optional: Index on JSONB if you frequently query based on included modifier IDs
-- CREATE INDEX idx_location_menu_items_enabled_modifier_ids ON location_menu_items USING gin (enabled_modifier_ids);
...
CREATE INDEX idx_location_ingredient_costs_location_id ON location_ingredient_costs (location_id); -- Index FK
CREATE INDEX idx_location_ingredient_costs_ingredient_id ON location_ingredient_costs (ingredient_id); -- Index FK
...
CREATE INDEX idx_inventory_stock_location_id ON inventory_stock (location_id); -- Index FK
CREATE INDEX idx_inventory_stock_ingredient_id ON inventory_stock (ingredient_id); -- Index FK
...
CREATE INDEX idx_inventory_movements_location_id ON inventory_movements (location_id); -- Index FK
CREATE INDEX idx_inventory_movements_ingredient_id ON inventory_movements (ingredient_id); -- Index FK
CREATE INDEX idx_inventory_movements_type ON inventory_movements (type);
CREATE INDEX idx_inventory_movements_created_at ON inventory_movements (created_at); -- Useful for time-based queries
CREATE INDEX idx_inventory_movements_recorded_by_staff_id ON inventory_movements (recorded_by_staff_id); -- Index FK

-- Optional: Triggers to automatically update `updated_at`
...
-- No updated_at trigger for inventory_movements as they are immutable historical records. created_at suffices.