...
jest.mock('@radix-ui/react-context-menu', () => {
  ...
  const Root = ({ children }) => <div data-testid="context-menu-root">{children}</div>;
  ...
  const CheckboxItem = React.forwardRef(({ children, checked, ...props }, ref) => (
    <div 
      ref={ref} 
  ...
  const RadioItem = React.forwardRef(({ children, ...props }, ref) => (
    <div ref={ref} data-testid="context-menu-radio-item" role="menuitemradio" {...props}>
      {children}
    </div>
  ));
  
  const ItemIndicator = ({ children }) => <div data-testid="context-menu-item-indicator">{children}</div>;
  ...
  const Separator = React.forwardRef((props, ref) => (
    <div ref={ref} data-testid="context-menu-separator" role="separator" {...props} />
  ));
  
  const Sub = ({ children }) => <div data-testid="context-menu-sub">{children}</div>;
  ...
  const Portal = ({ children }) => <div data-testid="context-menu-portal">{children}</div>;
  
  const Group = ({ children }) => <div data-testid="context-menu-group">{children}</div>;
...
describe('ContextMenu', () => {
  it('renders basic context menu structure', () => {
    ...
    expect(screen.getByTestId('context-menu-trigger')).toHaveTextContent('Right click me');
  ...
  it('renders checkbox items correctly', () => {
    render(
      <ContextMenu>
        <ContextMenuTrigger>Right click me</ContextMenuTrigger>
        <ContextMenuContent>
          <ContextMenuCheckboxItem checked={true}>Option 1</ContextMenuCheckboxItem>
          <ContextMenuCheckboxItem checked={false}>Option 2</ContextMenuCheckboxItem>
        </ContextMenuContent>
      </ContextMenu>
    );
  ...
  it('renders submenu structure', () => {
    ...
    expect(screen.getByTestId('context-menu-sub-trigger')).toHaveTextContent('More Options');
  ...
  it('renders shortcuts', () => {
    ...
    expect(shortcuts[0]).toHaveClass('ml-auto', 'text-xs', 'tracking-widest', 'text-muted-foreground');
  });
  ...
  it('applies custom classes to components', () => {
    ...
    expect(screen.getByTestId('context-menu-content')).toHaveClass('custom-content');
    expect(screen.getByTestId('context-menu-item')).toHaveClass('custom-item');
    expect(screen.getByTestId('context-menu-separator')).toHaveClass('custom-separator');
    expect(screen.getByTestId('context-menu-label')).toHaveClass('custom-label');
  });
  ...
  it('handles inset prop correctly', () => {
    ...
    expect(screen.getByText('Inset Item').closest('[data-testid="context-menu-item"]')).toHaveClass('pl-8');
    expect(screen.getByText('Inset Label').closest('[data-testid="context-menu-label"]')).toHaveClass('pl-8');
    expect(screen.getByText('Inset Trigger').closest('[data-testid="context-menu-sub-trigger"]')).toHaveClass('pl-8');
  });
});