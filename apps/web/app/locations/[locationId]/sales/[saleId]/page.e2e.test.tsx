...
test.describe('SaleDetailPage', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/locations/123/sales/456')
  })

  test('renders all required elements', async ({ page }) => {
    // Check that all elements are visible
    await expect(page.getByTestId('sale-detail-page')).toBeVisible()
    await expect(page.getByTestId('sale-detail-title')).toBeVisible()
    await expect(page.getByTestId('sale-detail-content')).toBeVisible()
  })
  ...
  test('takes a screenshot of the page', async ({ page, browserName }) => {
    ...
    await page.screenshot({ 
      path: `./screenshots/sale-detail_${browserName}_${timestamp}.png`,
      fullPage: true 
    })
  })
}) 